...
router.post('/register', async (req, res) => {
    ...
    try {
        ...
        // Hash password (using bcrypt default, which is $2b$)
        // Note: PHP used $2y$, but $2b$ is better for new users.
...
// Profile Route (Detailed user info + Favorites)
...
router.put('/profile', async (req, res) => {
    ...
    try {
        ...
        if (currentPassword) {
            // PHP used $2y$, Node used $2b$. Convert if necessary for comparison.
            let passwordHash = user.password;
            if (passwordHash && passwordHash.startsWith('$2y$')) {
                passwordHash = passwordHash.replace(/^\$2y\$/, '$2b$');
            }
...
router.post('/forgot-password', async (req, res) => {
    ...
    // Simulated: In real app, send email with token
